...
contract Swapper is OwnableUpgradeable, ERC20Upgradeable {
  ...
  bytes32 public constant WERTC = keccak256('WERTC');
  ...
  function initialize(MarketRateOracle _marketRate) public initializer {
    ...
    swapFeePct = 10000000000000000000;
  }
  ...
  function requestSwap(
    ...
    wrappedFromAddress.transferFrom(_msgSender(), adminAddress[_fromTokenType], _fromAmount);
  ...
  function updateMarketRateOracleAddress(address _marketRateAddress)
    external
    onlyOwner
    returns (bool)
  {
  ...
  function setTokenContractAddress(bytes32 TOKEN_TYPE, address tokenAddress)
    external
    onlyOwner
    returns (bool)
  {
  ...
  function setTokenAdminAddress(bytes32 TOKEN_TYPE, address _adminAddress)
    external
    onlyOwner
    returns (bool)
  {
